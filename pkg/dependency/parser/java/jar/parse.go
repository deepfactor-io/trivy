...
func (p *Parser) parseArtifact(filePath string, size int64, r xio.ReadSeekerAt) ([]ftypes.Package, []ftypes.Dependency, error) {
	...
	if manifestProps.Valid() {
		...
		if err == nil {
			...
			return append(pkgs, gavProps.Package()), nil, nil
		} else {
			p.logger.Debug("manifestProps: failed to get info from javadb using GAV", log.Err(err))
		}
	}
	...
	if err == nil {
		...
		p.logger.Debug("failed to get info from javadb using SHA1", log.Err(err))
	}
...
func (p *Parser) traverseZip(filePath string, size int64, r xio.ReadSeekerAt, fileProps Properties) (
	...
	for _, fileInJar := range zr.File {
		...
		case filepath.Base(fileInJar.Name) == "pom.properties":
			...
			lib := props.Package()
			if err == nil {
				lib.Licenses = append(lib.Licenses, gavProps.Package().Licenses...)
			} else {
				p.logger.Debug("pomProperties: failed to get info from javadb using GAV", log.Err(err))
			}
