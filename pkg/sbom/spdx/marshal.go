...
func (m *Marshaler) MarshalReport(ctx context.Context, report types.Report) (*spdx.Document, error) {
	...
	return m.Marshal(ctx, bom, report.DfScanMeta)
}

func (m *Marshaler) Marshal(ctx context.Context, bom *core.BOM, dfscanMeta types.DfScanMeta) (*spdx.Document, error) {
	var (
	...
	return &spdx.Document{
		...
		DocumentNamespace: getDocumentNamespace(root, dfscanMeta),
		CreationInfo: &spdx.CreationInfo{
			...
			Created: dfscanMeta.Created.UTC().Format(time.RFC3339),
		},
		Packages:      packages,
...
func getDocumentNamespace(root *core.Component, dfscanMeta types.DfScanMeta) string {
	return fmt.Sprintf("%s/%s/%s-%s",
		DocumentNamespace,
		string(root.Type),
		strings.ReplaceAll(strings.ReplaceAll(root.Name, "https://", ""), "http://", ""), // remove http(s):// prefix when scanning repos
		dfscanMeta.ScanID, // overriden for consistency
	)
}
