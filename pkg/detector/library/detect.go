...
func detect(ctx context.Context, driver Driver, pkgs []ftypes.Package) ([]types.DetectedVulnerability, error) {
	...
	for _, pkg := range pkgs {
		...
		for i := range vulns {
			...
			vulns[i].PkgDev = pkg.Dev
			vulns[i].PkgIndirect = pkg.Indirect
			vulns[i].PkgRootDependencies = pkg.RootDependencies
			vulns[i].NodeDedupeMatchFound = pkg.NodeDedupeMatchFound
